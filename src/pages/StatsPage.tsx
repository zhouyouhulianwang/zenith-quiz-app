      ...
      <div style={{ position: "relative", zIndex: 1, padding: "16px", paddingBottom: "100px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", gap: "6px" }}>
            {["本周", "本月", "全部"].map((r) => (
              <button key={r} onClick={() => setTimeRange(r)} style={{ padding: "5px 12px", borderRadius: "16px", fontSize: "12px", border: "none", cursor: "pointer", background: timeRange === r ? "#00d4ff" : "var(--card-bg-secondary)", color: timeRange === r ? "var(--page-bg)" : "var(--text-secondary)" }}>{r}</button>
            ))}
          </div>
        </div>
