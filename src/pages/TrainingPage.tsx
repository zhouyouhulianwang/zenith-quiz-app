    ...
    <div style={{ position: "relative", minHeight: "100dvh", background: "var(--page-bg)", overflowX: "hidden" }}>
      ...
      <div style={{ position: "relative", zIndex: 1, padding: "16px", paddingBottom: "100px" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>
          {inProgress.length > 0 && (
            <div>
              <div style={{ fontSize: "11px", fontWeight: 500, color: "var(--accent-color)", textTransform: "uppercase", letterSpacing: "1.5px", marginBottom: "8px", paddingLeft: "4px" }}>继续练习 · {inProgress.length}</div>
          ...
          {completed.length > 0 && (
            <div>
              <div style={{ fontSize: "11px", fontWeight: 500, color: "var(--success)", textTransform: "uppercase", letterSpacing: "1.5px", marginBottom: "8px", paddingLeft: "4px" }}>已完成 · {completed.length}</div>
...
function BankCard({ bank, onSelect }: { bank: { id: number; title: string; color: string; progress: number; questionsJson: string; chaptersJson: string | null; recCount: number; wrongCount: number }; onSelect: (id: number) => void }) {
  ...
  return (
    ...
    >
      <div style={{ width: "4px", height: "40px", borderRadius: "2px", background: bank.progress >= 100 ? "var(--success)" : bank.color || "var(--accent-color)", flexShrink: 0 }} />
          ...
          {bank.progress > 0 && bank.progress < 100 && (
            <>
              <div style={{ flex: 1, height: "3px", background: "var(--card-bg-secondary)", borderRadius: "2px", maxWidth: "80px" }}>
                <div style={{ width: `${bank.progress}%`, height: "100%", background: bank.color || "var(--accent-color)", borderRadius: "2px" }} />
              </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
          {chapterStats.map((ch, i) => (
            <motion.button key={ch.chapterId} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
              ...
              <div style={{ width: "40px", height: "40px", borderRadius: "10px", background: `${bank.color || "var(--accent-color)"}15`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <FileText size={18} color={bank.color || "var(--accent-color)"} />
              </div>
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px", marginTop: "3px" }}>
                  ...
                  {ch.totalInChapter > 0 ? (
                    <>
                      ...
                      {ch.wrong > 0 && <span style={{ fontSize: "11px", color: "var(--error)" }}>{ch.wrong} 错误</span>}
                    </>
                  ) : (
    ...
    >
      <button
        ...
        style={{
          ...
          whiteSpace: "nowrap",
          flexShrink: 0,
          background: currentChapterId === undefined ? bankColor || "var(--accent-color)" : "var(--card-bg-secondary)",
      ...
      {chapters.map((ch, idx) => (
        <button
          ...
          style={{
            ...
            whiteSpace: "nowrap",
            flexShrink: 0,
            background: currentChapterId === ch.chapterId ? bankColor || "var(--accent-color)" : "var(--card-bg-secondary)",
    ...
    <div style={{ position: "relative", minHeight: "100dvh", background: "var(--page-bg)" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
            <button
              ...
              style={{
                ...
                borderRadius: "8px", padding: "6px 10px", color: "var(--accent-color)",
        ...
        <div style={{ width: "100%", height: "3px", background: "var(--card-bg-secondary)" }}>
          <motion.div animate={{ width: `${progress}%` }} transition={{ duration: 0.3 }} style={{ height: "100%", background: "var(--accent-color)" }} />
        </div>
        ...
        {chapters.length > 0 && (
          <ChapterBar
            ...
            bankColor={bankData.color || "var(--accent-color)"}
          />
        )}
        ...
        <div style={{ display: "flex", gap: "5px", padding: "10px 16px", overflowX: "auto", WebkitOverflowScrolling: "touch" }}>
          {answers.map((ans, idx) => {
            ...
            if (idx === currentIndex) { bg = "var(--accent-color)"; border = "1px solid #00d4ff"; }
            ...
            return (
              <button key={idx} onClick={() => { setSwipeDir(idx > currentIndex ? "left" : "right"); setCurrentIndex(idx); }}
                style={{
                  ...
                  color: idx === currentIndex ? "var(--page-bg)" : ans.submitted ? (ans.isCorrect ? "var(--success)" : "var(--error)") : "var(--text-tertiary)",
        ...
        <div style={{ padding: "0 16px calc(140px + env(safe-area-inset-bottom))", userSelect: "none", WebkitUserSelect: "none" }} onTouchStart={handleTouchStart} onTouchEnd={handleTouchEnd}>
          <motion.div key={currentIndex} custom={swipeDir}
            ...
            <div style={{ background: "var(--card-bg)", borderRadius: "16px", padding: "20px", border: "1px solid rgba(255,255,255,0.08)", marginBottom: "16px" }}>
              <div style={{ display: "inline-flex", alignItems: "center", gap: "8px", padding: "4px 12px", borderRadius: "8px", background: "rgba(0,212,255,0.15)", color: "var(--accent-color)", fontSize: "12px", fontWeight: 500, marginBottom: "12px" }}>
                {typeLabel}
                ...
                {currentAnswer?.submitted && <span style={{ color: currentAnswer.isCorrect ? "var(--success)" : "var(--error)" }}>{currentAnswer.isCorrect ? " ✓ 正确" : " ✗ 错误"}</span>}
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              {displayOptions.map((option, index) => {
                ...
                let borderColor = "rgba(255,255,255,0.08)", bgColor = "var(--card-bg-secondary)", circleColor = "var(--text-tertiary)";
                if (submitted) {
                  if (isCorrect) { borderColor = "var(--success)"; bgColor = "rgba(16,185,129,0.1)"; circleColor = "var(--success)"; }
                  else if (isSelected && !isCorrect) { borderColor = "var(--error)"; bgColor = "rgba(239,68,68,0.1)"; circleColor = "var(--error)"; }
                } else if (isSelected) { borderColor = "var(--accent-color)"; bgColor = "rgba(0,212,255,0.1)"; circleColor = "var(--accent-color)"; }
                return (
                  <motion.button key={`${currentIndex}-${index}`} whileTap={!submitted ? { scale: 0.98 } : undefined} onClick={() => handleSelect(index)}
                    ...
                    <div style={{ width: "22px", height: "22px", borderRadius: currentQuestion?.type === "multiple" ? "4px" : "50%", border: `2px solid ${circleColor}`, background: isSelected ? circleColor : "transparent", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: "1px" }}>
                      {isSelected && <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}>{currentQuestion?.type === "multiple" ? <Check size={14} color="var(--page-bg)" strokeWidth={3} /> : <div style={{ width: "8px", height: "8px", borderRadius: "50%", background: "var(--page-bg)" }} />}</motion.div>}
                    </div>
                    <div style={{ flex: 1 }}>
            ...
            {currentQuestion?.type === "multiple" && !currentAnswer?.submitted && (
              <motion.button whileTap={{ scale: 0.95 }} onClick={handleMultiSubmit} disabled={!currentAnswer?.selected.length}
                style={{
                  ...
                  background: currentAnswer?.selected.length ? "var(--accent-color)" : "var(--card-bg-secondary)",
            ...
            {currentAnswer?.submitted && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} transition={{ duration: 0.3 }} style={{ overflow: "hidden", marginTop: "16px" }}>
                <div style={{ background: currentAnswer.isCorrect ? "rgba(16,185,129,0.1)" : "rgba(239,68,68,0.1)", border: `1px solid ${currentAnswer.isCorrect ? "rgba(16,185,129,0.3)" : "rgba(239,68,68,0.3)"}`, borderRadius: "12px", padding: "16px" }}>
                  <div style={{ fontSize: "14px", fontWeight: 600, color: currentAnswer.isCorrect ? "var(--success)" : "var(--error)", marginBottom: "6px" }}>
        ...
        <div style={{
          ...
          <motion.button whileTap={{ scale: 0.95 }} onClick={handleNext}
            style={{
              flex: 1, padding: "14px", borderRadius: "12px", background: "var(--accent-color)",
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.85)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <motion.div initial={{ scale: 0.8 }} animate={{ scale: 1 }} transition={{ type: "spring", damping: 20 }}
            ...
            <div style={{ display: "flex", justifyContent: "center", marginBottom: "24px" }}>
              <div style={{ width: "140px", height: "140px", borderRadius: "50%", border: "6px solid #2a2a2a", borderTopColor: accuracy >= 80 ? "var(--success)" : accuracy >= 60 ? "var(--accent-color)" : "var(--error)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", transform: "rotate(-90deg)" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "24px" }}>
              ...
              <div style={{ textAlign: "center", background: "rgba(16,185,129,0.1)", borderRadius: "10px", padding: "12px", border: "1px solid rgba(16,185,129,0.2)" }}><div style={{ fontSize: "20px", fontWeight: 700, color: "var(--success)" }}>{correctCount}</div><div style={{ fontSize: "12px", color: "var(--text-secondary)" }}>正确数</div></div>
              <div style={{ textAlign: "center", background: "rgba(239,68,68,0.1)", borderRadius: "10px", padding: "12px", border: "1px solid rgba(239,68,68,0.2)" }}><div style={{ fontSize: "20px", fontWeight: 700, color: "var(--error)" }}>{wrongCount}</div><div style={{ fontSize: "12px", color: "var(--text-secondary)" }}>错误数</div></div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
              <motion.button whileTap={{ scale: 0.95 }}
                ...
                style={{ padding: "14px", borderRadius: "12px", background: "var(--accent-color)", color: "var(--page-bg)", fontSize: "14px", fontWeight: 600, border: "none", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", minHeight: "48px", touchAction: "manipulation" }}>
