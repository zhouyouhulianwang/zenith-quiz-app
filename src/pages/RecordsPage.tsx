  ...
  return (
    <div style={{ position: "relative", minHeight: "100dvh", background: "var(--page-bg)" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ width: "100%", height: "3px", background: "var(--card-bg-secondary)" }}><motion.div animate={{ width: `${total > 0 ? ((currentIndex + 1) / total) * 100 : 0}%` }} style={{ height: "100%", background: "var(--accent-color)" }} /></div>

        {bankOptions.length > 1 && (
          <div style={{ padding: "10px 16px", display: "flex", gap: "8px", overflowX: "auto" }}>
            <button onClick={() => { setFilterBankId(null); setCurrentIndex(0); }} style={{ padding: "5px 12px", borderRadius: "16px", fontSize: "12px", border: "none", cursor: "pointer", whiteSpace: "nowrap", background: filterBankId === null ? "var(--accent-color)" : "var(--card-bg-secondary)", color: filterBankId === null ? "var(--page-bg)" : "var(--text-secondary)" }}><Filter size={11} style={{ display: "inline", marginRight: "3px", verticalAlign: "middle" }} /> 全部 {(records || []).length}条</button>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "24px", padding: "12px 16px" }}>
          ...
          <div style={{ textAlign: "center" }}><div style={{ fontSize: "18px", fontWeight: 700, color: "var(--success)" }}>{correctCount}</div><div style={{ fontSize: "11px", color: "var(--text-tertiary)" }}>正确</div></div>
          <div style={{ textAlign: "center" }}><div style={{ fontSize: "18px", fontWeight: 700, color: "var(--error)" }}>{wrongCount}</div><div style={{ fontSize: "11px", color: "var(--text-tertiary)" }}>错误</div></div>
        </div>
          ...
          {current && question && (
            <motion.div key={`${current.bankId}-${current.questionId}-${current.createdAt.getTime()}`} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.25 }}>
              <div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "12px", flexWrap: "wrap" }}>
                ...
                <span style={{ fontSize: "11px", padding: "3px 10px", borderRadius: "6px", background: current.isCorrect ? "rgba(16,185,129,0.15)" : "rgba(239,68,68,0.15)", color: current.isCorrect ? "var(--success)" : "var(--error)", fontWeight: 500, display: "flex", alignItems: "center", gap: "3px" }}>{current.isCorrect ? <CheckCircle size={10} /> : <XCircle size={10} />}{current.isCorrect ? "正确" : "错误"}</span>
              ...
              <div style={{ background: "var(--card-bg)", borderRadius: "16px", padding: "20px", border: "1px solid rgba(255,255,255,0.08)" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                  {question.options.map((opt, idx) => {
                    ...
                    const color = isCorrect ? "var(--success)" : isSelected && !isCorrect ? "var(--error)" : "var(--text-secondary)";
                    return (
                      <div key={idx} style={{ padding: "10px 12px", borderRadius: "8px", background: bg, border: `1px solid ${isCorrect ? "rgba(16,185,129,0.3)" : isSelected && !isCorrect ? "rgba(239,68,68,0.3)" : "rgba(255,255,255,0.05)"}`, fontSize: "14px", color, display: "flex", alignItems: "center", gap: "8px" }}>
                        <span style={{ width: "22px", height: "22px", borderRadius: "50%", background: isCorrect ? "var(--success)" : isSelected ? "var(--error)" : "var(--text-tertiary)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 600, color: "var(--text-primary)" }}>{String.fromCharCode(65 + idx)}</span>
        ...
        <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, padding: "12px 16px", paddingBottom: "max(16px, env(safe-area-inset-bottom))", background: "rgba(26,26,26,0.98)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderTop: "1px solid rgba(255,255,255,0.08)", display: "flex", gap: "12px", zIndex: 100 }}>
          ...
          <motion.button whileTap={{ scale: 0.95 }} onClick={handleNext} disabled={currentIndex >= total - 1} style={{ flex: 1, padding: "14px 20px", borderRadius: "12px", background: currentIndex >= total - 1 ? "var(--card-bg-secondary)" : "var(--accent-color)", color: currentIndex >= total - 1 ? "var(--text-tertiary)" : "var(--page-bg)", fontSize: "14px", fontWeight: 600, border: "none", cursor: currentIndex >= total - 1 ? "not-allowed" : "pointer", minHeight: "48px", touchAction: "manipulation", userSelect: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: "6px" }}>下一条 <ChevronRight size={18} /></motion.button>
        </div>
      </div>
    </div>
  );
