    ...
    return (
      <div style={{ position: "relative", minHeight: "100vh", background: "#1a1a1a", display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
          ...
          <p style={{ color: "#a0a0a0", marginTop: "16px" }}>暂无练习记录</p>
  ...
  return (
    <div style={{ position: "relative", minHeight: "100dvh", background: "#1a1a1a" }}>
      ...
      <div style={{ position: "relative", zIndex: 1 }}>
        ...
        <div style={{ width: "100%", height: "3px", background: "#2a2a2a" }}><motion.div animate={{ width: `${total > 0 ? ((currentIndex + 1) / total) * 100 : 0}%` }} style={{ height: "100%", background: "#00d4ff" }} /></div>

        {bankOptions.length > 1 && (
          <div style={{ padding: "10px 16px", display: "flex", gap: "8px", overflowX: "auto" }}>
            <button onClick={() => { setFilterBankId(null); setCurrentIndex(0); }} style={{ padding: "5px 12px", borderRadius: "16px", fontSize: "12px", border: "none", cursor: "pointer", whiteSpace: "nowrap", background: filterBankId === null ? "#00d4ff" : "#2a2a2a", color: filterBankId === null ? "#1a1a1a" : "#a0a0a0" }}><Filter size={11} style={{ display: "inline", marginRight: "3px", verticalAlign: "middle" }} /> 全部 {(records || []).length}条</button>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: "24px", padding: "12px 16px" }}>
          <div style={{ textAlign: "center" }}><div style={{ fontSize: "18px", fontWeight: 700, color: "#fff" }}>{total}</div><div style={{ fontSize: "11px", color: "#666" }}>总记录</div></div>
          <div style={{ textAlign: "center" }}><div style={{ fontSize: "18px", fontWeight: 700, color: "#10b981" }}>{correctCount}</div><div style={{ fontSize: "11px", color: "#666" }}>正确</div></div>
          <div style={{ textAlign: "center" }}><div style={{ fontSize: "18px", fontWeight: 700, color: "#ef4444" }}>{wrongCount}</div><div style={{ fontSize: "11px", color: "#666" }}>错误</div></div>
        </div>
                  ...
                  {question.options.map((opt, idx) => {
                    ...
                    return (
                      <div key={idx} style={{ padding: "10px 12px", borderRadius: "8px", background: bg, border: `1px solid ${isCorrect ? "rgba(16,185,129,0.3)" : isSelected && !isCorrect ? "rgba(239,68,68,0.3)" : "rgba(255,255,255,0.05)"}`, fontSize: "14px", color, display: "flex", alignItems: "center", gap: "8px" }}>
                        <span style={{ width: "22px", height: "22px", borderRadius: "50%", background: isCorrect ? "#10b981" : isSelected ? "#ef4444" : "#666", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 600, color: "#fff" }}>{String.fromCharCode(65 + idx)}</span>
                        <span>{displayOpt}</span>
                      </div>
                    );
                  })}
                </div>
                {question.explanation && <div style={{ marginTop: "16px", padding: "12px", borderRadius: "10px", background: "#1a1a1a" }}><div style={{ fontSize: "12px", color: "#666" }}>解析</div><div style={{ fontSize: "13px", color: "#a0a0a0", lineHeight: 1.6 }}>{question.explanation}</div></div>}
              </div>
            </motion.div>
          )}
        </div>
